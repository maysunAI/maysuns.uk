...
## 2026-07-27 T00介绍页（t00.html）内容扩充

用户反馈"T00是我最重要的appeal，现在的介绍不吸引人，图也太少"，提出几个具体问题要求解答+扩充。已改动`t00.html`+`assets/i18n.js`（三语言ja/en/zh同步）：

- **新增"两个我经常被问到的问题"板块**：①日语老师问的"AI能不能不用一个个打开文件就完成任务"——用这次对话本身作为真实例证（用户只说了一句要求，AI自己找文件、读、改、核对，全程没人手动开过文件）；②"是否只限于T00文件夹内"——如实说明T00只是规则/记录的存放地，不是AI活动范围的围墙，给路径就能操作电脑上任何文件，但重要操作（删除/公开发布）仍会先确认。
- **新增"T00一目了然"板块**：Before/After两栏对比图（CSS/HTML实现，不是AI生成的图片——本次会话没有接入图片生成服务，用代码画的信息图表达同样"一目了然"的效果，且以后改文字不用重新生成图）。
- **新增"两个角度看T00"板块**：面向非技术背景（今天该做什么/重要事项提醒/帮读文件/理解意图不跑偏/拿不准先问）+面向开发者（token消耗/长对话跑偏/后期细节问题/**核心的"前80%快、后20%质量掉或需要更多人力"问题**，逐条对应T00的具体解决机制）。
- **备份**：改动前把`t00.html`/`knowledge-t00.html`/`assets/i18n.js`复制到`Z01_backup_20260727/`。
- **验证**：`node --check`确认`i18n.js`语法正确（改动中一度误用直角引号导致JS字符串提前截断，已发现并改成全角「」引号修复）；Python脚本核对`t00.html`的`<div>`标签开闭数量一致（44对44）。**本次会话没有可用浏览器工具，未做Playwright真机渲染验证**，建议部署后自己打开页面确认视觉效果正常。
- **部署**：本地改完后git commit(`49db405`)+push到`github.com/maysunAI/maysuns.uk`，再scp上传到Vultr服务器`/var/www/pj50-website/`。已用curl核对线上`t00.html`/`assets/i18n.js`的`Last-Modified`时间戳和新增内容片段，确认改动已生效（`https://maysuns.uk/t00.html`）。

## 2026-07-27 反馈的4个手机端bug排查+首页文案重写

用户反馈iPhone上4个问题+首页T00介绍不吸引人。逐条排查+修复：

1. **"微信发送后用Edge打开有问题"+"语言切换后没生效"**：代码核查排除了"缺翻译key"（ja/zh各594条一一对应）和"硬编码日文没走i18n"（全站html文件扫描无未受i18n保护的日文文本节点）两个可能，**但发现了一个真实问题**：上一轮（前面这条记录）部署`t00.html`+`assets/i18n.js`时改了文件内容，却没有同步把`?v=20260720c`这个缓存版本号跟着改——CDN/浏览器缓存可能一直用旧版本号命中旧缓存，看起来就像"改了没生效"。**已修复**：全站28个html文件的`site.css`/`i18n.js`/`main.js`版本号统一升级到`?v=20260727`，强制所有客户端重新拉取最新文件。
2. **"AI知识库菜单下拉后首页跑到中间位置"**：`.nav-links`手机端只有`flex-wrap`没有纵向堆叠，下拉菜单展开时高度突然变化，把同一行后面的"首页"链接挤到换行、插进展开菜单中间。已改成`flex-direction: column`纵向堆叠，展开菜单只会把它下面的项目往下推，不会打乱前面顺序。
3. **"AI产品/AI动态链接有时候不好用"**：找到`.nav-dropdown-menu`原来同时有CSS `:hover`触发和JS点击(`.open`类)触发两套显示逻辑——这是iOS Safari/Edge一个已知问题的诱因：触屏第一次点击可能被浏览器当成"模拟hover"吃掉，需要点第二次才真正触发JS，表现就是"有时候点了没反应"。已改成只保留JS点击这一套逻辑（`.open`类），去掉CSS hover触发，桌面鼠标用户体验从"划过去展开"变成"点一下展开"（原来的点击逻辑本来就已经支持，只是被hover规则干扰了）。
4. **首页介绍重写**：原来首页顶部只有一句口号"Real products. Real experience. Real feelings."，没有任何说明T00是什么/能做什么。已重写成：新eyebrow"T00到底能做什么"+新主标题"说出你想要的，它自己找文件、动手做、还会自己核对"+一段说明文字（不用一个个开文件/不用重复解释背景/AI自己找文件读改/有风险操作会先问）+两个按钮（"看看T00能做什么"跳t00.html、"看看已经做出了什么"跳作品区），原来的口号降级成小字副标题保留。三语言（日/英/中）同步。
