...
- 未执行任何 `git push` 或服务器部署操作，改动目前只在本地工作目录。
- 未改动任何支付/API Key/密钥相关配置。

## 状态

✅ 12条需求中11条已完整落地，1条（#11 md内容查看功能）因现有代码中确无实现痕迹，已如实标注为待用户确认，未杜撰。已用Playwright实测：首页/About/产品详情页在三语言下正确渲染、语言切换顺序正确、主题切换正常、桌面与移动宽度均无横向溢出、导航跳转正常、吃药提醒App正确标注"开发中"且无死链接。截图见 `help_screenshots/`。尚未部署到线上（未push/未改GitHub Pages配置），改动目前仅在本地。
